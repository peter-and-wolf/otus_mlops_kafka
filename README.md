...
В папке `src` живет код:

* producer.py – простой продьюсер на основе пакета [confluent-kafka](https://github.com/confluentinc/confluent-kafka-python), оформлен в виде утилиты командной строки, принимающей следующие ключи: 
  * `topic` – имя топика, в который писать (по-умолчаию, `payments`);
  * `limit` – сколько сообщений записать, если `None` (значение по-умолчанию), пишет, пока вы его не остановите;
  * `idempotence` – по-умолчанию – `False`, если `True`, продьюсер станет идемпотентным (будет стараться делать сообщения уникальными и не отправлять повторно те, что уже отправил); 
  * `retries` – количество (по-умплчанию – 2) повторных отправок сообщений, которые продьюсер счел "недоставленными";
  * `acks` – уровень "подтверждения" отправки, любо из значений `no` (подтверждение не требуется вовсе), `one` – нужно единственное подтверждение от брокера-лидера партиции, `all` – нужно подтверждение от [`min.insync.replicas`](https://learn.conduktor.io/kafka/kafka-topic-configuration-min-insync-replicas/) брокеров;
* consumer.py – простой консьюмер на основе пакета [confluent-kafka](https://github.com/confluentinc/confluent-kafka-python), оформлен в виде утилиты командной строки, принимающей следующие ключи: 
  * `topic` – имя топика, из которого читать (по-умолчаию, `payments`);
  * `limit` – сколько сообщений считать, если `None` (значение по-умолчанию), читает, пока вы его не остановите; 
  * `group` – имя группы, в которую будет включен консьюмер;
  * `offset` – смещение, начиная с которого читать сообщения;
  * `auto_commit` – `True` или `False` – включена или нет автоматическая фиксация смещений;
  * `manual_commit` – `True` или `False` – фиксировать или нет смещения вручную, если автоматическая фиксация отключена;
